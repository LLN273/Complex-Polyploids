...
### Script used to polarize polyploid sequence in a MSA.
...
try:
    POLYPID = sys.argv[4]                  				# polyploid sequence ID  
except:
    print('\n Error: provide the following arguments:')
    print('01s_polarizeTETRA.py [input.fasta] [No. species] [refSequence ID] [polyploid ID] \n')
    exit() 
...
###################################################### Unfold polyploid and ref sequences
...
## Add polyploid and ref sequence to new dictionary
DICT_seq = dict()	
...
###################################################### Create polarized sequence for polyploid
...
species_list_aux.remove(POLYPID)	# remove polyploid
