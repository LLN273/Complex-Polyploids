...
for i in `seq 1 1 $N_GENES`; do                 # cycle through genes
   ...
   GO_GENE=${GENE_LIST[$i]}
   GO_MSA=${GO_GENE}.${InFileSUF}
   #echo $i $GO_MSA

   # NO SLIM
   #AUX_fa=$GO_MSA
   #AUX_prefix_out=${GO_MSA%.fa}_UFBoot_${OFE}
   #rsync -ah $AA/$GO_MSA $SNIC_TMP/$AUX_fa

   # Remove MAN02, No NAN01, No PENDULA_GERMANY.KYYNEL sequences (redundant)
   AUX_fa=${GO_MSA%.fa}_SLIM-NoMAN02NoNAN01.fa
   AUX_prefix_out=${GO_MSA%.fa}_UFBoot_${OFE}_SLIM-NoMAN02NoNAN01
   sed '/^>MAN02/{N;d}' $AA/$GO_MSA > $SNIC_TMP/AUX2_fa
   sed '/^>NAN01/{N;d}' $SNIC_TMP/AUX2_fa > $SNIC_TMP/AUX3_fa
   sed '/^>PENDULA_GERMANY.KYYNEL/{N;d}' $SNIC_TMP/AUX3_fa > $SNIC_TMP/$AUX_fa
...
#    example.phy.splits.nex: 	support values in percentage for all splits (bipartitions), computed as the occurence frequencies in the bootstrap trees. 
#				This file can be viewed with the program SplitsTree to explore the conflicting signals in the data. So it is more informative than consensus tree, e.g. you can
#				see how highly supported the second best conflicting split is, which had no chance to enter the consensus tree.
...
# Branch support is shown for both SH-aLRT and ultrafast bootstrap, in the following order:
