...
echo "VCF file (indels):" $AA/${SNAME}-JG_indels.vcf.gz
...
echo
echo '1. Preparing variants file'
...
#awk '$5 != "*" { print $0 }' $SNIC_TMP/${SNAME}_variants_aux2.vcf >> $SNIC_TMP/${SNAME}_variants.vcf		# excludes alternate sites marked as '*' 
awk '$5 !~ /*/ { print }' $SNIC_TMP/${SNAME}_variants_aux2.vcf >> $SNIC_TMP/${SNAME}_variants.vcf		# excludes alternate sites that include '*' ('*' but also 'A,*' sites etc -- but this is a tiny number) >> necessary otherwise these sites are REMOVED from consensus seqeunce !!
...
##### 3. Create consensus sequence using IUPAC codes for all genotypes
echo
...
awk '$5 !~ /*/ { print }' $SNIC_TMP/${SNAME}_snap-mask_aux2.txt >> $SNIC_TMP/${SNAME}_snpmask.vcf		# excludes alternate sites that include '*' ('*' but also 'A,*' sites etc -- but this is a tiny number) >> necessary otherwise these sites are REMOVED from consensus seqeunce !!
...
VCF_indels=${SNAME}-JG_indels.vcf.gz
zcat $AA/$VCF_indels | grep '^#' > $SNIC_TMP/VCF_indels1.vcf
zcat $AA/$VCF_indels | grep -v '^#' | grep PASS | grep -vE "([.]/[.])" > $SNIC_TMP/__aux_pass1.txt
awk '$5 !~ /*/ { print }' $SNIC_TMP/__aux_pass1.txt > $SNIC_TMP/__aux_pass2.txt		    # excludes alternate sites that include '*' ('*' but also 'A,*' sites etc -- but this is a tiny number) >> necessary otherwise these sites are REMOVED from consensus seqeunce !!
...
#### 5. Use bedtools to mask ./. sites, failed variants, and deletions

echo
echo '5a. Running bedtools maskfasta, SNP'
bedtools maskfasta \
