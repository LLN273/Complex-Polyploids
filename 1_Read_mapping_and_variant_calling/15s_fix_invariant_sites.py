...
for line in fhand:
    ...
    if len(xx) > 0 : 
        ...
        if FLAG_PLOIDY == 1 :
            ...
            PLY_aux1 = PLY_aux1.split(':', 1)[0]   # get genotype
            PLY_aux2 = PLY_aux1.count('/') 
            if PLY_aux2 == 0 : PLY_aux2 = PLY_aux1.count('|')
            if PLY_aux2 == 1 :      # diploid (eg 0/0)
                GT_PASS = '0/0'
                GT_FAIL = './.'
            elif PLY_aux2 == 2 :      # triploid (eg 0/0/0)
                GT_PASS = '0/0/0'
                GT_FAIL = '././.'
        ...
        if (ALT != '.') :
            ...
            try:
                ...
                # if read depth not listed in genotype fields, try INFO field    >>>> DO NOT USE; coverage in INFO field is not updated after extracting sample from multisample VCF if there is no DP information in the genotype fields
                #INFOf = x[7]                            # read INFO field
                #try:
                #    DP = float(getValue(INFOf, 'DP='))	    # get read depth from INFO fields (DP), if present
                #except:
                #    DP = 0                              # if read depth not listed, set DP to zero
            #
            if DP == 0 :
                ...
                x[9]=':'.join(GT_split)             # reformat genotype fileds to VCF format
