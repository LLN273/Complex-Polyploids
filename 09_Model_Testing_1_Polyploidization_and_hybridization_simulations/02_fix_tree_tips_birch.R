...
## read sample list
sample_list <- read.table(SPL,header=F)



# For each species, check whether both individuals form a monophyletic group. if not, remove one of the individuals.

# run for each species in tree
id_kept <- c()
...
for (i in c(1:NSPEC)){			
   ...
   IND1 <- paste(sample_list[i,1],"_0_0",sep='')
   IND2 <- paste(sample_list[i,1],"_0_1",sep='')
   #print(IND1)
...
## Set minimum branch length to 0.01
